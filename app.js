...
let tickTimer = null;      // steady 1s clock for elapsed/active time
let lastWall = 0;          // Date.now() of the previous clock tick
let locked = false;
...
function render() {
  ...
  el.elapsed.textContent = formatDuration(state.activeMs);
}
...
// steady clock: counts active (foreground) time so elapsed + overall average
// keep advancing even when the GPS is quiet because you're standing still
function startClock() {
  if (tickTimer != null) return;
  lastWall = Date.now();
  tickTimer = setInterval(() => {
    const now = Date.now();
    state.activeMs += now - lastWall;
    lastWall = now;
    render();
    saveState();
  }, 1000);
